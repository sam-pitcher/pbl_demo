...
embed_secret = 'good_one'

############################
#  URL GENERATOR FUNCTION  #
############################

def generate(dashboard_id=23):
  user = User(
    '999',
    first_name='Embed',
    last_name='Looker',
    permissions=["see_lookml_dashboards", "access_data", "see_looks", "see_user_dashboards", "see_drill_overlay"],
    models=["strava"],
    user_attributes={"country": "UK"}
  )

  looker = Looker('pitcherpakeman001.lookersandbox.com', embed_secret)

  fifteen_minutes = 15 * 60

  url = URL(looker, user, fifteen_minutes, f"/embed/dashboards/{dashboard_id}?embed_domain=http://127.0.0.1:5000", force_logout_login=True)
